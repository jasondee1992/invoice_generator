...
HEADER_LOGO_RELATIVE_PATH = Path("logo/Logo.png")
WATERMARK_LOGO_RELATIVE_PATH = Path("logo/watermark_logo.png")
WATERMARK_DRAW_WIDTH = 500
WATERMARK_DRAW_HEIGHT = 500
HEADER_LOGO_DRAW_WIDTH = 130
HEADER_LOGO_DRAW_HEIGHT = 130
...
def build_logo_reader(
    ...
    """Prepare a logo image for PDF drawing."""
...
def draw_page_watermark(pdf: canvas.Canvas) -> None:
    """Draw a large faint logo watermark centered on the page."""
    logo_path = get_existing_asset_path(WATERMARK_LOGO_RELATIVE_PATH)
    if not logo_path:
        return
...
def draw_header_logo(
    ...
    x: float,
    y: float,
    width: float = HEADER_LOGO_DRAW_WIDTH,
    height: float = HEADER_LOGO_DRAW_HEIGHT,
) -> None:
    """Draw a crisp logo on the first invoice page."""
    logo_path = get_existing_asset_path(HEADER_LOGO_RELATIVE_PATH)
    if not logo_path:
        return
...
def draw_invoice_top_sections(pdf: canvas.Canvas, invoice: dict, page_number: int) -> float:
    ...
    if page_number == 1:
        ...
        pdf.drawCentredString(PAGE_WIDTH / 2, top_y, "INVOICE")
        pdf.setFillColor(TEXT_COLOR)

        left_x = MARGIN
        right_x = PAGE_WIDTH / 2 + 36
        title_to_content_gap = 54
        section_gap = 110
        billed_by_top = top_y - title_to_content_gap
        bill_to_top = billed_by_top - section_gap
        sender_line_count = max(1, len(invoice["sender_block"].splitlines()))
        sender_block_top = billed_by_top
        sender_text_top = billed_by_top - 18
        sender_text_bottom = sender_text_top - (sender_line_count * 13)
        sender_block_bottom = sender_text_bottom - 2
        sender_block_center_y = (sender_block_top + sender_block_bottom) / 2

        pdf.setFont("Helvetica-Bold", 11)
        pdf.drawString(left_x, billed_by_top, "Billed By")
        draw_text_block(
            pdf,
        ...
        pdf.setFont("Helvetica-Bold", 11)
        pdf.drawString(left_x, bill_to_top, "Bill To")
        draw_text_block(
            pdf,
        ...
        draw_header_logo(
            pdf,
            x=PAGE_WIDTH - MARGIN - HEADER_LOGO_DRAW_WIDTH - 2,
            y=sender_block_center_y - (HEADER_LOGO_DRAW_HEIGHT / 2),
            width=HEADER_LOGO_DRAW_WIDTH,
            height=HEADER_LOGO_DRAW_HEIGHT,
        )

        invoice_info_top = bill_to_top - 20
        value_x = PAGE_WIDTH - MARGIN
        row_gap = 15

        pdf.setFont("Helvetica", 10)
        pdf.drawString(right_x, invoice_info_top, "Invoice Number")
        pdf.drawRightString(value_x, invoice_info_top, invoice["invoice_number"])
        pdf.drawString(right_x, invoice_info_top - row_gap, "Invoice Date")
        pdf.drawRightString(value_x, invoice_info_top - row_gap, invoice["invoice_date"])
        pdf.drawString(right_x, invoice_info_top - (row_gap * 2), "Due Date")
        pdf.drawRightString(value_x, invoice_info_top - (row_gap * 2), invoice["due_date"])

        return invoice_info_top - (row_gap * 2) - 54
